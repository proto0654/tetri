<div>
    @php
        $contactPhone = collect(app(\App\Settings\SiteSettings::class)->get('phones') ?? [])
            ->pluck('number')
            ->filter()
            ->first();
        $contactPhoneTel = filled($contactPhone)
    ...
    >
        ...
        >
            <div class="flex items-start justify-between gap-4">
                <div>
                    <h2 id="booking-modal-title" class="font-display text-3xl font-normal leading-tight text-olive sm:text-4xl">
                        @if ($sent)
            ...
            @if ($sent)
                <div class="mt-6 space-y-4">
                    <p class="text-sm text-ink">
                        Мы скоро свяжемся с вами, чтобы подтвердить бронь.
                    </p>
                    <p class="text-xs text-muted">
                        Бронь считается подтверждённой после связи с менеджером.
                    </p>
                    <button
                        type="button"
                        wire:click="close"
            ...
            @elseif ($failed)
                <div class="mt-6 space-y-4" role="alert">
                    <p class="text-sm text-ink">
                        Заявку сейчас не получилось отправить. Попробуйте ещё раз или позвоните нам — данные в форме сохранены.
                    </p>
                    @if (filled($contactPhone) && filled($contactPhoneTel))
                        <p class="text-sm text-ink">
                            <a href="tel:{{ $contactPhoneTel }}" class="font-medium text-plum transition hover:text-plum-dark">
                                {{ $contactPhone }}
                            </a>
                        </p>
                    @endif
                    <div class="flex flex-col gap-3 sm:flex-row">
                        <button
                            type="button"
                            wire:click="retry"
                            class="w-full rounded-full bg-plum px-6 py-3 text-sm font-semibold tracking-wide text-white transition hover:bg-plum-dark sm:flex-1"
                        >
                            Попробовать снова
                        </button>
                        <button
                            type="button"
                            wire:click="close"
                            class="w-full rounded-full border border-ink/15 bg-transparent px-6 py-3 text-sm font-semibold tracking-wide text-ink transition hover:bg-cream-dark sm:flex-1"
                        >
                            Закрыть
                        </button>
                    </div>
